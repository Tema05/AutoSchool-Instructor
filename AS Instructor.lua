    ...
    then
	 if version ~= thisScript().version
	  then
	   ...
	   downloadUrlToFile("", thisScript().path, function(id, status, p1, p2)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA
         then 
	      sampAddChatMessage("[SMIHelper] {FFFFFF}Загрузка обновления V" .. updateversion.v .. " завершена.", 0x3399FF)
        end
       end)
	  else
